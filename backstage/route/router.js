...
//const searchDayInfo = require('../api/zhihuishu.js')
...
// 查看当日小纸条
router.get('/everyday', (req, res,next) => {
  mongo.searchDay({dayInfo:req.query.dayInfo}).then(data => {
    if (data == null) {
      console.log("每日纸条查询失败")
      res.json({status: 0})
    }else {
      console.log("每日纸条查询成功")
      console.log(data)
      res.json({status: 1,data})
    }
  }).catch(err => {
